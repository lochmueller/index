...
use Lochmueller\Index\Traversing\Extender\Address;
...
class AddressTest extends AbstractTest
{
    ...
    {
        $subject = new Address($this->createStub(RecordSelection::class));

        self::assertInstanceOf(ExtenderInterface::class, $subject);
    }

    public function testGetNameReturnsAddress(): void
    {
        $subject = new Address($this->createStub(RecordSelection::class));

        self::assertSame('address', $subject->getName());
    }
    ...
    public function testGetItemsReturnsEmptyIterableWhenNoRecordsFound(): void
    {
        ...
        $subject = new Address($recordSelectionStub);
    ...
    public function testGetItemsUsesCorrectArgumentsForAddressPlugin(): void
    {
        $recordStub = $this->createStub(Record::class);
        ...
        $routerMock->expects(self::once())
            ...
            ->with(
                ...
                self::callback(fn(array $arguments): bool => $arguments['_language'] === $languageStub
                        && $arguments['tx_ttaddress_listview']['action'] === 'show'
                        && $arguments['tx_ttaddress_listview']['controller'] === 'Address'
                        && $arguments['tx_ttaddress_listview']['address'] === 123),
            )
    ...
    public function testGetItemsUsesEmptyArrayWhenRecordStoragesNotSet(): void
    {
        ...
        $subject = new Address($recordSelectionMock);
