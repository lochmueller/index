...
use Lochmueller\Index\Traversing\Extender\News;
...
class NewsTest extends AbstractTest
{
    ...
    {
        $subject = new News($this->createStub(RecordSelection::class));

        self::assertInstanceOf(ExtenderInterface::class, $subject);
    }

    public function testGetNameReturnsNews(): void
    {
        $subject = new News($this->createStub(RecordSelection::class));

        self::assertSame('news', $subject->getName());
    }
    ...
    public function testGetItemsReturnsEmptyIterableWhenNoRecordsFound(): void
    {
        ...
        $subject = new News($recordSelectionStub);
    ...
    public function testGetItemsUsesCorrectArgumentsForNewsPlugin(): void
    {
        $recordStub = $this->createStub(Record::class);
        ...
        $routerMock->expects(self::once())
            ...
            ->with(
                ...
                self::callback(fn(array $arguments): bool => $arguments['_language'] === $languageStub
                        && $arguments['tx_news_pi1']['action'] === 'detail'
                        && $arguments['tx_news_pi1']['controller'] === 'News'
                        && $arguments['tx_news_pi1']['news'] === 99),
            )
    ...
    public function testGetItemsUsesCorrectTableName(): void
    {
        ...
        $recordSelectionMock->expects(self::once())
            ...
            ->with('tx_news_domain_model_news', [15], 0)
            ->willReturn([]);

        $subject = new News($recordSelectionMock);
